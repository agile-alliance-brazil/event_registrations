# encoding: UTF-8
require File.join(Rails.root, "/lib/token_generator")
require File.join(Rails.root, "/lib/trimmer")
...
class RegistrationGroup < ActiveRecord::Base
  ...
  validates_format_of :contact_email, :with => /\A[^@\s]+@([^@\s]+\.)+[^@\s]+\z/, :allow_blank => true
  ...
  scope :for_event, lambda { |c| select('DISTINCT registration_groups.*').joins(:attendees).where('attendees.event_id = ?', c.id) }
