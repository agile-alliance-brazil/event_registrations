RSpec.describe ReportsController, type: :controller do
  context 'unauthenticated' do
    describe 'GET #attendance_organization_size' do
      before { get :attendance_organization_size, event_id: 'foo' }
      it { expect(response).to redirect_to login_path }
    end

    describe 'GET #attendance_years_of_experience' do
      before { get :attendance_years_of_experience, event_id: 'foo' }
      it { expect(response).to redirect_to login_path }
    end
  ...
  context 'authenticated as organizer' do
    ...
    context 'and is organizing the event' do
      ...
      describe 'GET #attendance_years_of_experience' do
        ...
        it 'assign the instances variables and renders the template' do
          ...
          get :attendance_years_of_experience, event_id: event
      ...
      describe 'GET #attendance_job_role' do
        ...
        it 'assign the instances variables and renders the template' do
          ...
          get :attendance_job_role, event_id: event
    ...
    context 'and is not organizing the event' do
      describe 'GET #attendance_organization_size' do
        let(:event) { FactoryGirl.create :event }
        before { get :attendance_organization_size, event_id: event }
        it { expect(response).to have_http_status :not_found }
      end

      describe 'GET #attendance_years_of_experience' do
        let(:event) { FactoryGirl.create :event }
        before { get :attendance_years_of_experience, event_id: event }
        it { expect(response).to have_http_status :not_found }
      end

      describe 'GET #attendance_job_role' do
        let(:event) { FactoryGirl.create :event }
        before { get :attendance_job_role, event_id: event }
        it { expect(response).to have_http_status :not_found }
      end
    end
  end
