  ...
  describe '#cancel' do
    context 'when having one active event for today' do
      ...
      context 'and one attendance accepted and advised 7 days ago with gateway as payment type' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, status: :accepted, advised: true, advised_at: 7.days.ago, due_date: 7.days.ago) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'gateway') }
      ...
      context 'and one attendance from 10 days ago not advised' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, advised_at: 10.days.ago, advised: false) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'gateway') }
      ...
      context 'and one attendance older than 15 days with bank_deposit as payment type' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, advised_at: 15.days.ago, advised: true) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'bank_deposit') }
        it 'notifies the pending attendance' do
          EmailNotifications.expects(:cancelling_registration).never
          notifier.cancel
        end
      ...
      context 'and one attendance older than 15 days with statement of agreement as payment type' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, advised_at: 15.days.ago, advised: true) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'statement_agreement') }
        it 'notifies the pending attendance' do
          EmailNotifications.expects(:cancelling_registration).never
          notifier.cancel
        end
  ...
  describe '#cancel_warning' do
    context 'when having one active event for today' do
      ...
      context 'and one attendance older than 7 days' do
        context 'with gateway as payment type' do
          ...
          context 'and has been already advised' do
            let!(:attendance) { FactoryBot.create(:attendance, event: event, last_status_change_date: 7.days.ago, advised: true, advised_at: Time.zone.today) }
            let!(:invoice) { Invoice.from_attendance(attendance, 'gateway') }
            it 'notifies the pending attendance and mark as advised' do
      ...
      context 'and one accepted attendance older than 14 days with gateway as payment type' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, last_status_change_date: 7.days.ago, status: :accepted) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'gateway') }
      ...
      context 'and one attendance older than 13 days with gateway as payment type' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, last_status_change_date: 6.days.ago) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'gateway') }
        it 'notifies the pending attendance' do
          EmailNotifications.expects(:cancelling_registration_warning).never
          notifier.cancel_warning
      ...
      context 'and one attendance older than 15 days with bank_deposit as payment type' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, last_status_change_date: 7.days.ago) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'bank_deposit') }
        it 'notifies the pending attendance' do
          EmailNotifications.expects(:cancelling_registration_warning).never
          notifier.cancel_warning
      ...
      context 'and one attendance older than 15 days with statement of agreement as payment type' do
        let!(:attendance) { FactoryBot.create(:attendance, event: event, last_status_change_date: 8.days.ago) }
        let!(:invoice) { Invoice.from_attendance(attendance, 'statement_agreement') }
        it 'notifies the pending attendance' do
          EmailNotifications.expects(:cancelling_registration_warning).never
          notifier.cancel_warning
