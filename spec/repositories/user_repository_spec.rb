describe UserRepository, type: :repository do
  describe '#search_engine' do
    let!(:user) { FactoryGirl.create :user, first_name: 'Foo', last_name: 'bar', email: 'foo@bar.com', roles_mask: 1, updated_at: 1.day.from_now }
    let!(:other_user) { FactoryGirl.create :user, first_name: 'Sbbrubles', last_name: 'bar', email: 'sbbrubles@bar.com', roles_mask: 1, updated_at: Time.zone.today }
    ...
    let!(:forth_user) { FactoryGirl.create :user, first_name: 'Foo', last_name: 'xpto', email: 'sbbrubles@xpto.com', roles_mask: 3, updated_at: 1.day.ago }
