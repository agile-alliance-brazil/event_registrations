...
RSpec.describe AttendanceHelper, type: :helper do
  ...
  describe '#job_role_options' do
    it { expect(job_role_options).to eq(Attendance.job_roles.map { |job_role, key| [I18n.t("activerecord.attributes.attendance.enums.job_role.#{job_role}"), key] }.sort_by { |roles| roles[0] }) }
  end
end
